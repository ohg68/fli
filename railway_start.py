...
input,select{background:var(--sf2);border:1px solid var(--bd);border-radius:8px;padding:11px 12px;color:var(--t);font-family:'DM Sans',sans-serif;font-size:15px;outline:none;transition:border-color .2s;-webkit-appearance:none;appearance:none;width:100%}
...
.dc{background:var(--sf);border:1px solid var(--bd);border-radius:var(--rd);padding:14px 16px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;animation:fi .35s ease-out both}
.dd{font-size:15px;font-weight:500}.dp{font-family:'Space Mono',monospace;font-size:18px;font-weight:700;color:var(--ac)}
.dch .dp{color:var(--g)}
...
["Singapore","SIN","Singapore"],["Bangkok","BKK","Thailand"],
["Sydney","SYD","Australia"],["Johannesburg","JNB","South Africa"]
];

function acS(q){
  q=q.toLowerCase().trim();if(q.length<2)return[];
  var r=[];
  for(var i=0;i<DB.length;i++){var d=DB[i];if(d[0].toLowerCase().indexOf(q)>=0||d[1].toLowerCase()===q||d[2].toLowerCase().indexOf(q)>=0)r.push(d)}
  if(q.length===3&&r.length===0)r.push([q.toUpperCase(),q.toUpperCase(),'']);
  return r.slice(0,8);
}

function sAC(iid,lid){
  var inp=$(iid),list=$(lid);
  inp.addEventListener('input',function(){
    var res=acS(inp.value);
    if(!res.length){list.classList.remove('open');return}
    var h='';for(var i=0;i<res.length;i++){var d=res[i];h+='<div class="aci" data-c="'+d[1]+'"><span class="code">'+d[1]+'</span> '+d[0]+'<br><small>'+d[2]+'</small></div>'}
    list.innerHTML=h;list.classList.add('open');
    var items=list.querySelectorAll('.aci');
    for(var j=0;j<items.length;j++){items[j].addEventListener('click',function(){inp.value=this.getAttribute('data-c');inp.setAttribute('data-r',this.getAttribute('data-c'));list.classList.remove('open')})}
  });
...
function bL(o,d,date,ret){
  ...
  var ky='https://www.kayak.com/flights/'+o+'-'+d+'/'+date;
  if(ret)ky+='/'+ret;
  return '<div class="links"><a href="'+gf+'" target="_blank">Google Flights</a><a href="'+sk+'" target="_blank">Skyscanner</a><a href="'+ky+'" target="_blank">Kayak</a></div>';
}
...
async function mp(b,n){
  var h={'Content-Type':'application/json','Accept':'application/json, text/event-stream'};
  if(sid)h['Mcp-Session-Id']=sid;
  var res=await fetch(MCP,{method:'POST',headers:h,body:JSON.stringify(b)});
  var s=res.headers.get('Mcp-Session-Id');if(s)sid=s;
  if(n){try{await res.text()}catch(e){}return null}
  var txt=await res.text();var lines=txt.replace(/\r/g,'').split('\n');var results=[];
  for(var i=0;i<lines.length;i++){var ln=lines[i].trim();if(ln.indexOf('data: ')===0){try{results.push(JSON.parse(ln.substring(6)))}catch(e){}}}
  if(results.length>0)return results[results.length-1];
  try{return JSON.parse(txt)}catch(e){return null}
}

async function connect(){
  $('st').textContent='connecting...';$('sd').classList.remove('on');
  try{
    var r=await mp({jsonrpc:'2.0',id:++rid,method:'initialize',params:{protocolVersion:'2025-03-26',capabilities:{},clientInfo:{name:'fli-web',version:'2.0'}}});
    if(!r||!r.result)throw new Error('x');
    await mp({jsonrpc:'2.0',method:'notifications/initialized'},true);
    conn=true;$('st').textContent='connected';$('sd').classList.add('on');$('bf').disabled=false;$('bd').disabled=false;
  }catch(e){$('st').textContent='offline';setTimeout(connect,5000)}
}
...
async function sF(){
  ...
  var o=gC('fo'),d=gC('fd');
  var p={origin:o,destination:d,departure_date:$('ff').value,cabin_class:$('fc2').value,max_stops:$('fs').value,sort_by:$('ft').value,passengers:parseInt($('fp').value)||1};
  if($('fret').value)p.return_date=$('fret').value;
  try{var res=await mp({jsonrpc:'2.0',id:++rid,method:'tools/call',params:{name:'search_flights',arguments:p}});rF(res,p)}
...
async function sD(){
  ...
  var p={origin:o,destination:d,start_date:$('dfr').value,end_date:$('dto').value,trip_duration:parseInt($('du').value)||5,is_round_trip:$('dr').value==='true',cabin_class:$('dc2').value,max_stops:$('ds').value,sort_by_price:true,passengers:1};
  try{var res=await mp({jsonrpc:'2.0',id:++rid,method:'tools/call',params:{name:'search_dates',arguments:p}});rD(res,p)}
  catch(e){$('ra').innerHTML='<div class="msg"><div class="ic">&#9888;</div>'+e.message+'</div>'}
...
function rF(res,p){
  try{
    ...
    if(!data){$('ra').innerHTML='<div class="msg"><div class="ic">&#128269;</div>No results.</div>';return}
    if(data.success===false){$('ra').innerHTML='<div class="msg"><div class="ic">&#9888;</div>'+(data.error||'Error')+'</div>';return}
    var fl=data.flights;if(!fl||!fl.length){$('ra').innerHTML='<div class="msg"><div class="ic">&#128269;</div>No flights found.</div>';return}
...
function rD(res,p){
  try{
    ...
    if(!data){$('ra').innerHTML='<div class="msg"><div class="ic">&#128269;</div>No results.</div>';return}
    if(data.success===false){$('ra').innerHTML='<div class="msg"><div class="ic">&#9888;</div>'+(data.error||'Error')+'</div>';return}
    var dates=data.dates;if(!dates||!dates.length){$('ra').innerHTML='<div class="msg"><div class="ic">&#128269;</div>No dates.</div>';return}
    ...
    for(var i=0;i<dates.length;i++){
      ...
      h+='<div class="dc'+(ch?' dch':'')+'" style="animation-delay:'+(i*.04)+'s"><div><div class="dd">'+(d.date||'--')+'</div>';
      if(d.return_date)h+='<div style="font-size:12px;color:var(--t3)">Return: '+d.return_date+'</div>';
      h+='</div><div class="dp">'+(ch?'&#9733; ':'')+'$'+d.price+'</div></div>';
    }
    $('ra').innerHTML=h;
  }catch(e){$('ra').innerHTML='<div class="msg"><div class="ic">&#9888;</div>'+e.message+'</div>'}
}
